...
const ViewSwitcher = class {
	...
	add (name: string, view: any) {
		if (this.views[name] === void 0) {
			if (view instanceof View) this.views[name] = view
			else console.error(`${view} is not an instance of \`View\``)
		}
		else console.error(`${name} already exists`)
	}

	remove (name: string) {
		if (typeof this.views[name] === 'undefined') console.error(`${name} doesn't exist`)
		else delete this.views[name]
	}

	open (name: string, cb = () => {}) {
		if (this.views[name] === void 0) console.error(`${name} doesn't exist`)
			...
			switch (this.state) {
				...
				case 'open': {
					if (this.currentView === name) console.log('Open was called on the view already open')
					else {
						this.state = 'closing'
	...
	close (name?: string, cb = () => {}) {
		...
		switch (this.state) {
			...
			case 'open': {
				...
				else if (this.views[name] === void 0) console.log(`${name} doesn't exist`)
				...
				else console.log(`${this.views[name]} is not open`)
